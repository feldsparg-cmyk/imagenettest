...
if st.session_state.get("demo_mode_toggle", False):
    st.error("이 모드에서는 편향성을 학습한 AI를 보여주기 위해 대상의 특징을 혐오 단어로만 표시합니다.", icon="🚨")

st.markdown("<br><hr>", unsafe_allow_html=True)
st.subheader("📖 ImageNet Roulette와 데이터 편향성 논란")
st.markdown("""

■ 인공지능의 보이지 않는 구조에 질문을 던지는 트레버 페글렌, LG 구겐허임 어워드 수상자로 선정
인공지능(AI)이 당신의 외모를 분석해 한 단어로 규정한다면 어떨까. 피부색과 성별, 옷차림만으로 당신은 범죄자, 알코올 중독자라는 낙인이 붙을 수도 있다. 트레버 페글렌은 2019년 관객 참여형 프로젝트 ‘이미지넷 룰렛(ImageNet Roulette)’을 진행하며 AI가 인간을 분류하는 방식에 내재된 편견을 가감 없이 드러냈다. 
...
트레버 페글렌은 이미지넷의 판단 알고리즘을 그대로 가져와 사람들이 직접 자신의 셀카를 업로드하면, 인공지능이 어떻게 사람을 분류하는지를 실시간으로 보여줬다. 흑인 남성은 범죄자, 용의자로 분류되고, 안경 쓴 사람은 괴짜, 공부벌레 같은 라벨이 붙는 식이다.
그는 오염된 학습 데이터 속 AI가 내리는 판단에 인간이 가진 편견과 인종 차별이 녹아들어가 있음을 경고했다. 

소셜 미디어를 통해 확산되며 논란이 거세지자,결국 이미지넷을 관리하던 연구팀은 **2019년 9월, 공식 사과와 함께 1,593개의 혐오·차별적 표현을 데이터베이스에서 전면 삭제**했다. 해당 카테고리에서만 60만장의 사진을 삭제되었고 분류 체계가 전면 수정되며 AI 업계에 변화를 이끌어 냈다.

AI 등 기술의 시선에 질문을 던져온 트레버 페글렌이 ‘LG 구겐하임 어워드’ 수상자로 선정된 것은 우연이 아니다. LG 역시 기술이 단순히 효율성을 높이는 도구를 넘어 인간의 삶에 어떤 영감과 영향을 주는지를 고민해 왔기 때문이다.
