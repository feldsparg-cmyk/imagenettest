...
st.markdown("""
2019년, 아티스트 트레버 페글렌(Trevor Paglen)과 AI 연구자 케이트 크로포드(Kate Crawford)가 공개한 **ImageNet Roulette** 프로젝트는 인공지능 학계와 대중에게 큰 충격을 주었습니다. 이 프로젝트는 방대한 이미지 학습 데이터인 '이미지넷(ImageNet)'에 내재된 차별적 시선을 폭로하기 위해, 사용자의 사진을 올리면 AI가 사람을 어떻게 분류하고 라벨링하는지 직접 체험할 수 있도록 설계되었습니다. 

가벼운 장난처럼 시작된 이 룰렛은 사용자의 얼굴을 '실패자', '범죄자', '매춘부', 심지어는 특정 인종을 비하하는 단어와 매칭하며, 인공지능이 과거 인류의 편견과 혐오를 얼마나 무분별하게 학습했는지를 적나라하게 보여주었습니다.

결국 프로젝트가 소셜 미디어를 통해 확산되며 논란이 거세지자, 이미지넷 측은 문제의 심각성을 인정했습니다. 그 결과 **2019년 9월, 438개의 '안전하지 않은(unsafe)' 카테고리와 1,155개의 '민감한(sensitive)' 카테고리를 포함해 총 1,593개의 혐오·차별적 표현이 데이터베이스에서 전면 삭제**되었습니다. 이와 함께 해당 라벨에 속해 있던 **약 60만 장 이상의 인물 이미지도 영구적으로 제거**되며, 전 세계 AI 개발자들에게 '학습 데이터의 윤리'라는 과제를 남겼습니다.
""")
