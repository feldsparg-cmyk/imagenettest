...
.stToggle label p { white-space: nowrap !important; word-break: keep-all; margin-bottom: 0; } 
...
with col2:
    # 띄어쓰기를 줄바꿈 방지 공백(\u00A0)으로 강제 변환하여 글씨 쪼개짐 원천 차단
    toggle_text = "🚨 극단적 편향 모드 켜기 (부정적/편견 단어만 매칭)".replace(" ", "\u00A0")
    st.toggle(toggle_text, key="demo_mode_toggle")
...
<br><br>
LG 관계자는 <b>"트레버 페글렌의 작품에 투영된 질문들은 LG가 해왔던 고민과 같은 선상에 있다"며 "LG 역시 AI 역량을 강화해 나감에 있어 투명성, 책임성, 그리고 기술의 인간 중심적 활용이 진정한 혁신의 기반이라고 믿는다"고 말했다. 이어 "그의 수상을 축하하며, 인간의 신뢰를 받을 수 있는 AI 미래를 구축하겠다는 LG의 의지를 다시 한번 다지는 계기가 될 다시 한번 다지는 계기가 될 것"</b>이라고 말했다.
</div>
""", unsafe_allow_html=True)
