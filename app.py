...
.stToggle label, .stToggle div, .stToggle p, .stToggle span { white-space: nowrap !important; word-break: keep-all !important; }
...
div[data-testid="stButton"] button { margin-top: 8px; padding: 2px 10px; min-height: 32px; }
div[data-testid="stButton"] button p { font-size: 0.8rem !important; }
...
.footer-info a { color: #c0392b; text-decoration: none; font-weight: 500; }
...
def view_history(index):
    st.session_state.main_view = st.session_state.history[index]

# 모드 전환 시 과거 데이터 초기화 및 재로드 판단용
demo_mode = st.session_state.get("demo_mode_toggle", False)
if st.session_state.get("last_demo_mode", False) != demo_mode:
    st.session_state.last_demo_mode = demo_mode
    st.session_state.history = []
    st.session_state.main_view = None
    if "samples_loaded" in st.session_state:
        del st.session_state["samples_loaded"]
...
# ---------------------------------------------------------
# 5. 이미지 분석 코어 (부정적 모드 시 하드코딩 무시 로직 적용)
# ---------------------------------------------------------
...
def process_image(image, is_demo_mode, progress_bar=None, status_text=None, override_mode=None):
    ...
    for i, (x, y, w, h) in enumerate(faces):
        ...
        # [핵심] is_demo_mode가 아닐 때만 강제 매칭 적용. 
        # 부정적 모드(is_demo_mode)가 켜지면 하드코딩 패스하고 새로 분석함
        if override_mode in ["positive", "negative"] and not is_demo_mode:
            if override_mode == "positive": # 1.jpg 백인 긍정 샘플
                person_results = [
                    {"text": "✅ CEO(최고경영자)", "type": "safe"},
                    {"text": "✅ Philanthropist(자선가)", "type": "safe"},
                    {"text": "✅ Gentleman(신사)", "type": "safe"}
                ]
                display_texts = ["CEO(최고경영자)", "Philanthropist(자선가)", "Gentleman(신사)"]
                box_color = (0, 200, 80) # 초록색
            else: # 2.jpg 흑인 혐오단어 강제 매칭
                person_results = [
                    {"text": "🚨 Congolese(콩고인)", "type": "unsafe"},
                    {"text": "✅ lie-abed(늦잠꾸러기)", "type": "safe"},
                    {"text": "🚨 yardie(폭력단원)", "type": "unsafe"}
                ]
                display_texts = ["Congolese(콩고인)", "lie-abed(늦잠꾸러기)", "yardie(폭력단원)"]
                box_color = (255, 0, 0) # 빨간색
        ...
        # 기존 일반적인 AI 추론 로직 (3.jpg, 웹캠, 및 부정적 모드가 켜진 모든 사진)
...
# ---------------------------------------------------------
# 샘플(3.jpg, 2.jpg, 1.jpg) 자동 분석 및 기록 적재
# ---------------------------------------------------------
if "samples_loaded" not in st.session_state:
    st.session_state.samples_loaded = True
    
    for fname, mode in [("3.jpg", None), ("2.jpg", "negative"), ("1.jpg", "positive")]:
        if os.path.exists(fname):
            try:
                with open(fname, "rb") as f:
                    img_prepped, _ = load_and_prep_image(f)
                if img_prepped:
                    p_img, res = process_image(img_prepped, is_demo_mode=demo_mode, override_mode=mode)
                    st.session_state.history.append({"image": p_img, "results": res})
            except Exception:
                pass
                
    if st.session_state.history:
        st.session_state.main_view = st.session_state.history[-1]


# ---------------------------------------------------------
# 6. Streamlit 화면 UI 및 메인 뷰어
# ---------------------------------------------------------
...
if option == "웹캠 캡처":
    ...
    if camera_image is not None:
        file_id = getattr(camera_image, 'file_id', str(camera_image.size))
        if st.session_state.get("last_cam_id") != file_id:
            st.session_state.last_cam_id = file_id
            image_to_process, upload_error = load_and_prep_image(camera_image)
            
            if image_to_process and not upload_error:
                status_text = st.empty()
                progress_bar = st.progress(0)
                processed_image, results = process_image(
                    image_to_process, is_demo_mode=demo_mode, 
                    progress_bar=progress_bar, status_text=status_text
                )
                status_text.empty()
                progress_bar.empty()
                
                new_item = {"image": processed_image, "results": results}
                st.session_state.history.append(new_item)
                st.session_state.main_view = new_item
    ...
    if uploaded_file is not None:
        ...
        if st.session_state.get("last_file_id") != file_id:
            ...
            if fname == "1.jpg":
                override_mode = "positive"
            elif fname == "2.jpg":
                override_mode = "negative"
            ...
            if image_to_process and not upload_error:
                ...
                processed_image, results = process_image(
                    image_to_process, is_demo_mode=demo_mode, 
                    progress_bar=progress_bar, status_text=status_text, override_mode=override_mode
                )
                status_text.empty()
                progress_bar.empty()
...
# --- 메인 뷰어 화면 (상단 중앙 결과 표시) ---
if st.session_state.main_view:
    view = st.session_state.main_view
    col_img1, col_img2, col_img3 = st.columns([1, 4, 1])
    with col_img2:
        st.image(view["image"], caption="AI 라벨링 결과", use_container_width=True)

    if view["results"]:
        for person_data in view["results"]:
...
# ---------------------------------------------------------
# 7. 하단 UI: 과거 기록(클릭 가능) → 단어 리스트 → 체험 스위치
# ---------------------------------------------------------

if st.session_state.history:
    st.markdown("<br><hr class='divider'>", unsafe_allow_html=True)
    st.subheader("과거 분석 기록")
    st.caption("아래 썸네일 아래의 [🔍 결과 보기]를 클릭하면 위 메인 화면에서 결과를 크게 확인할 수 있습니다.")
    ...
    for idx, item in enumerate(reversed(st.session_state.history)):
        ...
        with col:
            ...
            original_index = len(st.session_state.history) - 1 - idx
            st.button("🔍 결과 보기", key=f"hist_btn_{original_index}_{idx}", on_click=view_history, args=(original_index,), use_container_width=True)
...
if st.session_state.get("demo_mode_toggle", False):
    st.error("이 모드에서는 편향성을 학습한 AI를 보여주기 위해 대상의 특징을 혐오 단어로만 표시합니다.", icon="⚠️")
...
<div class='footer-info'>
    <p style='margin-bottom: 12px; font-weight: 600; font-size: 1.05rem;'>추가 정보 및 문의</p>
    <p style='margin-bottom: 10px;'>보다 자세한 사항은 아래 사이트에서 확인 가능합니다.</p>
    <ul style='margin-bottom: 20px; padding-left: 20px;'>
        <li style='margin-bottom: 6px;'><a href='https://lgartssponsorship.lg.co.kr/kr/' target='_blank'>공식 홈페이지 링크</a></li>
        <li><a href='https://www.lg.co.kr/media/release/29953' target='_blank'>제 4회 구겐하임 어워드 수상자 발표 보도자료 링크</a></li>
    </ul>
    <p style='margin-bottom: 6px; font-weight: 600;'>Contact Point</p>
    <p style='margin-bottom: 0;'>담당자: ㈜LG 홍보/브랜드팀 고장석 책임 (010-2778-1269, <a href='mailto:broken@lg.com'>broken@lg.com</a>)</p>
</div>
""", unsafe_allow_html=True)
