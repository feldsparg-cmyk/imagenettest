...
def process_image(image, is_demo_mode, progress_bar=None, status_text=None, override_mode=None):
    ...
    for i, (x, y, w, h) in enumerate(faces):
        ...
        # [핵심] override_mode에 따른 결과 강제 매칭 처리 (1.jpg, 2.jpg)
        if override_mode in ["positive", "negative"]:
        ...
        # 기존 일반적인 AI 추론 로직 (3.jpg, 웹캠 및 기타 이미지)
...
# ---------------------------------------------------------
# [추가] 초기 샘플(3.jpg, 2.jpg, 1.jpg) 자동 분석 및 기록 적재
# ---------------------------------------------------------
if "samples_loaded" not in st.session_state:
    st.session_state.samples_loaded = True
    
    # 순서대로 3, 2, 1을 넣어서 가장 마지막에 1.jpg가 추가되게 세팅
    for fname, mode in [("3.jpg", None), ("2.jpg", "negative"), ("1.jpg", "positive")]:
        if os.path.exists(fname):
            try:
                ...
                if img_prepped:
                    p_img, res = process_image(img_prepped, is_demo_mode=False, override_mode=mode)
                    st.session_state.history.append({"image": p_img, "results": res})
            except Exception:
                pass
                
    # 초기 앱 실행 시 맨 마지막에 적재된 (최근) 사진을 메인 뷰어에 표시
    ...
    if camera_image is not None:
        ...
        if st.session_state.get("last_cam_id") != file_id:
            ...
            if image_to_process and not upload_error:
                ...
                st.session_state.main_view = new_item # 방금 찍은 사진을 메인뷰로 세팅
    ...
    if uploaded_file is not None:
        ...
        if st.session_state.get("last_file_id") != file_id:
            ...
            if image_to_process and not upload_error:
                ...
                new_item = {"image": processed_image, "results": results}
                st.session_state.history.append(new_item)
                st.session_state.main_view = new_item # 방금 올린 사진을 메인뷰로 세팅
...
if st.session_state.history:
    ...
    for idx, item in enumerate(reversed(st.session_state.history)):
        ...
        with col:
            ...
            # 콜백을 통해 클릭 시 해당 기록의 인덱스 번호를 메인 뷰어로 전달
